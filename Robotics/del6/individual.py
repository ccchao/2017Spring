...
class INDIVIDUAL:
    ...
    def Compute_Fitness(self, env):
        ...
        y = self.sim.Get_Sensor_Data(sensorID=4) #robot -> first ball
        ...
        if env == 0:
            self.fitness = self.fitness + max(y) + m*20

        if env == 1:
            y = self.sim.Get_Sensor_Data(sensorID=9) #second ball -> first ball
            
            m = self.sim.Get_Sensor_Data(sensorID=10, s=2) #z position of second ball

            self.fitness = self.fitness + max(y)
